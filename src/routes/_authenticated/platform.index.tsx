...
export const Route = createFileRoute("/_authenticated/platform/")({
  head: () => ({
    meta: [
      { title: "ผู้ดูแลแพลตฟอร์ม · องค์กร | Document Hub" },
      { name: "description", content: "สร้างและจัดการองค์กรที่ใช้งานระบบ สำหรับผู้ดูแลแพลตฟอร์ม" },
      { property: "og:title", content: "ผู้ดูแลแพลตฟอร์ม · องค์กร" },
      { property: "og:description", content: "สร้างและจัดการองค์กรที่ใช้งานระบบ" },
    ],
  }),
  component: () => (
    <PlatformGuard title="องค์กร">
      <OrganizationsAdmin />
    </PlatformGuard>
  ),
});
...
function OrganizationsAdmin() {
  ...
  const save = useMutation({
    mutationFn: async (f: FormState) => {
      if (!f.code.trim() || !f.name.trim()) throw new Error("กรุณากรอกรหัสและชื่อองค์กร");
      const payload = {
        code: f.code.trim(),
        name: f.name.trim(),
      ...
      const { data, error } = await sb.from("organizations").insert(payload).select("id").single();
      if (error) throw error;
      const newId = (data as { id: string }).id;
      // เปิดเมนูมาตรฐานให้องค์กรใหม่
      await sb.from("organization_page_access").upsert(
        [
          "dashboard", "calendar", "notifications", "projects", "quotations",
          "partners", "documents", "contracts", "reports", "audit-log", "settings",
        ].map((page_key) => ({ organization_id: newId, page_key, enabled: true })),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="บริหารองค์กร"
        description="สร้างองค์กรใหม่ แก้ไขข้อมูล และเปิด/ปิดการใช้งานองค์กร"
        actions={
                ...
                <TableBody>
                  {orgs.map((o) => (
                    <TableRow key={o.id}>
                      ...
                      <TableCell className="hidden lg:table-cell text-sm">{o.phone || o.email || "—"}</TableCell>
                      <TableCell>
                        {o.is_active ? (
        ...
        <DialogContent className="max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="flex items-center gap-2 sm:col-span-2">
              ...
              <span className="text-sm">เปิดใช้งานองค์กรนี้</span>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpen(false)}>ยกเลิก</Button>
            <Button onClick={() => save.mutate(form)} disabled={save.isPending}>บันทึก</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
